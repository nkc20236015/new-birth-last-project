...
#pragma warning restore 0414
        ...
        public bool Exit(bool onGround, bool crouching)
        {
            ...
            if (condition == AttackCondition.MustBeOnGroundOrExit && (!onGround || crouching)) return true;
            if (condition == AttackCondition.MustBeInAirOrExit && onGround) return true;
            if (condition == AttackCondition.MustBeCrouchingOrExit && !crouching) return true;
            return false;
        }
        ...
        {
            if (condition == AttackCondition.Anytime) return false;
            if (condition == AttackCondition.MustBeOnGroundOrSkip && (!onGround || crouching)) return true;
            if (condition == AttackCondition.MustBeInAirOrSkip && onGround) return true;
            if (condition == AttackCondition.MustBeCrouchingOrSkip && !crouching) return true;
            return false;
        }

        public bool StateChanged(bool onGround, bool crouching)
        {
            if ((condition == AttackCondition.MustBeInAirOrExit || condition == AttackCondition.MustBeInAirOrSkip) && (onGround || crouching)) return true;
            if ((condition == AttackCondition.MustBeOnGroundOrExit || condition == AttackCondition.MustBeOnGroundOrSkip) && (!onGround || crouching)) return true;
            if ((condition == AttackCondition.MustBeCrouchingOrExit || condition == AttackCondition.MustBeCrouchingOrSkip) && (!onGround || !crouching)) return true;
            return false;
        }
    }
