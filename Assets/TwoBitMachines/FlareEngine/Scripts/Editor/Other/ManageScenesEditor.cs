...
namespace TwoBitMachines.FlareEngine.Editors
{
        ...
        public class ManageScenesEditor : UnityEditor.Editor
        {
                ...
                {
                        Block.Header(so).Style(FoldOut.boxColor).Label("Manage Scenes", 12, false, FoldOut.titleColor).Button().Build();
                        Block.boxRect.Tooltip("Add Transition/LoadScene");

                        int size = 2;// sceneNames == null || sceneNames.Count == 0 ? 2 : 4;
                        Block.Box(size, FoldOut.boxColor, extraHeight: 3, noGap: true);
                        {
                                // if (sceneNames != null)
                                // {
