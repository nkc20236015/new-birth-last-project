                ...
                private void ClimbWall (AbilityManager player, int wallDirection, ref Vector2 velocity)
                {
                        ...
                        if (slideDown != WallSlideType.None)
                        {
                                ...
                                if (sliding)
                                {
                                        ...
                                        {
                                                ImpactPacket impact = ImpactPacket.impact.Set(slideWE, player.world.transform, player.world.boxCollider, player.world.position, null, Vector2.right * wallDirection, 0);
                        ...
                        {
                                ResetStickTime();
                                onEnter.Invoke(ImpactPacket.impact.Set(enterWE, player.world.transform, player.world.boxCollider, player.world.position, null, Vector2.right * wallDirection, 0));
                        }
                        if ((!isHolding || velocity.y != 0) && onClimbRate > 0 && Clock.Timer(ref onClimbRateCounter, onClimbRate))
                        {
                                ImpactPacket impact = ImpactPacket.impact.Set(worldEffect, player.world.transform, player.world.boxCollider, player.world.position, null, Vector2.right * wallDirection, 0);
