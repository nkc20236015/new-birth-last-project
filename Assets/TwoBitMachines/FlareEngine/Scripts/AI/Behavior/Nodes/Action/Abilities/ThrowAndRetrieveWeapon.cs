        ...
        public class ThrowAndRetrieveWeapon : Action
        {
                ...
                public override NodeState RunNodeLogic (Root root)
                {
                        ...
                        if (state == RetrieveWeapon.HoldWeapon)
                        {
                                if (isRetrieving)
                                {
                                        decelerate = Mathf.Clamp01(decelerate + Time.deltaTime);
                ...
                private void RunWeapon ()
                {
                        if (weaponThrown)
                        {
                                ...
                                {
                                        weapon.transform.position += Vector3.right * throwDirection * throwSpeed * Time.deltaTime;
                                }
                        }
                        else if (weaponBouncing)
                        {
                                weapon.transform.position += Vector3.right * -throwDirection * throwSpeed * Time.deltaTime * 0.9f;
                        }
                        if (rotateWeapon && (weaponThrown || weaponBouncing))
                        {
                                weapon.transform.Rotate(Vector3.forward , rotateSpeed * 10f * Time.deltaTime);
                        }
