...
namespace TwoBitMachines
{
        public class TextMeshProEffects : MonoBehaviour
        {
                ...
                [SerializeField] private float waveStrength = 1f;

                [SerializeField] private float waveSpeedX = 2f;
                [SerializeField] private float wavePhaseX = 0.01f;
                [SerializeField] private float waveStrengthX = 1f;

                [SerializeField] private float jitterStrength = 1f;
                ...
                private void TypeWriter (ref bool update)
                {
                        ...
                        {
                                ImpactPacket impact = ImpactPacket.impact.Set (worldEffect, this.transform, null, this.transform.position, null, Vector2.zero, 0);
                ...
                {
                        TMP_TextInfo textInfo = textMesh.textInfo;
                        if (textInfo == null || textInfo.lineInfo == null) return;
                        ...
                        for (int l = 0; l < textInfo.linkCount; l++)
                        {
                                ...
                                else if (linkID == "wobble")
                                {
                                        Wobble (startIndex, range, textInfo);
                                }
                                else if (linkID == "distortion")
                                {
                ...
                private void Wobble (int startIndex, int range, TMP_TextInfo textInfo) //(float time)
                {
                        for (int i = startIndex; i < range; i++)
                        {
                                ...
                                float t = Time.time + i;
                                Vector2 effect = new Vector2 (Mathf.Sin (t * wobble), Mathf.Cos (t * wobble));
                ...
                {
                        if (!Clock.Timer (ref jitterCounter, jitterRate)) return;
                ...
                {
                        if (!Clock.Timer (ref distortionCounter, distortionRate)) return;
